                    ...
                    @forelse($medicines as $medicine)
                        <tr>
                            ...
                            <td class="d-flex gap-2 no-warp">
                                <!-- View -->
                                <div>
                                <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#viewMedicineModal{{ $medicine->id }}">
                                    <i class="bi bi-eye"></i>
                                </button>
                                </div>
                                <!-- Edit -->
                                <div>
                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editMedicineModal{{ $medicine->id }}">
                                ...
                                <!-- Delete -->
                                <div>
                                <form action="{{ route('hospital.management.medicine.destroy', $medicine->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this medicine?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <!-- Modals -->
                        @include('pages.AdminPages.hospital.modals.Medicine.view-medicine', ['medicine' => $medicine])
                        @include('pages.AdminPages.hospital.modals.Medicine.edit-medicine', ['medicine' => $medicine])
                    @empty
                        <tr>
